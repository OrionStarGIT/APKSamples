...
public class VirtualShareSource implements ZoomSDKShareSource {
    ...
    Runnable frameTask = new Runnable() {
        ...
        public void run() {
            ...
            videoFrame = readFrame(mVideoFile, mFrameIndex, mWidth, mHeight);
            mFrameIndex++;
            ...
            synchronized (mSenderLockObj) {
                if (mSender != null) {
                    mSender.sendShareFrame(videoFrame.byteBuffer, videoFrame.width, videoFrame.height, videoFrame.byteBuffer.capacity());
                }
            }
            mVirtualHandler.postDelayed(frameTask, 1000 / mFps);
        }
