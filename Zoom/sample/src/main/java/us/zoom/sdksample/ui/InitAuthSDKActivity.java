...
//        ZoomMeetingUISettingHelper.changeVideoSource(true, this);
    ...
    AudioCapManager mAudioRecer = null;
...
//            if(!isRecoding) {
//                mAudioRecer = new AudioCapManager();
//                int iRet = mAudioRecer.startRecord(16000, new AudioCapManager.AudioRecordCallback() {
//                    @Override
//                    public void onFrameDataIn(byte[] buffer, final boolean isMute) {
//                        Log.i(TAG, "get pcm date from auidorecord, len:" + buffer.length);
//                    }
//                });
//                Log.i(TAG, "start record rlt is:" + iRet);
//            } else {
//                mAudioRecer.stopRecord();
//            }
...
//        opts.invite_options = InviteOptions.INVITE_VIA_EMAIL + InviteOptions.INVITE_VIA_SMS;
    ...
    private void showMeetingUi() {
        if (ZoomSDK.getInstance().getMeetingSettingsHelper().isCustomizedMeetingUIEnabled()) {
            ...
            boolean enable = sharedPreferences.getBoolean("enable_rawdata", true);
            Intent intent = null;
            if (!enable) {
